...
Version 2.4: Length-adjusted canonical splice site detection for sequences with insertions/deletions
"""
...
def find_top2_scores_length_adjusted(donor_probs, hg38_length, current_seq_length, debug=False):
    ...
    - If n <= v: search positions [0, 59] (first 60 nt)
    - If n > v: search positions [0, adjusted_interval - 1] where adjusted_interval = 60 * (n / v)
    ...
    if current_seq_length <= hg38_length:
        # Case 1: n <= v, search first 60 nt
        search_end = min(60, len(donor_probs))
        if debug:
        ...
        adjusted_interval = int(60 * (current_seq_length / hg38_length))
...
def main():
    parser = argparse.ArgumentParser(description='Process MAF files with msa_view and SpliceAI (v2.4: length-adjusted canonical splice site detection)')
